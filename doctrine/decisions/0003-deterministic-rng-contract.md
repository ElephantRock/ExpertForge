...
3. The full digest identifies the stream. Fixed 64-bit and 32-bit projections are
   ...
   identity.
4. RNG mutation occurs only through explicit `RngManager.initialize()` or
   `RngManager.restore_state()` calls. Importing the package has no RNG or backend
   side effects.
5. Every run seeds Python's global RNG, NumPy's legacy global RNG, and an owned
   NumPy PCG64 `Generator`. NumPy is therefore runtime infrastructure.
6. Tensor frameworks integrate through explicit adapters. PyTorch is the first
   adapter and is imported lazily; it is not a mandatory dependency at this
   milestone.
7. `reproducible` mode requests deterministic provider behavior and uses either
   fail-fast or stable warning policy. `performance` mode is explicitly
   non-reproducible and may enable faster backend choices.
8. RNG state is a frozen, versioned, non-pickle Pydantic payload. Python and NumPy
   states are structural. Framework byte states use canonical base64 transport
   with SHA-256 verification. Issue #11 embeds this payload into checkpoints.
...
- Structural state models allow validation before mutation and direct deterministic
  JSON embedding in future checkpoints.
